<h1 align="center">Hi 👋, I'm Fatma Khaled</h1>

<h3 align="center">Frontend Developer | Angular Developer from Egypt 🇪🇬</h3>

<p align="center">
Passionate about building responsive, scalable, and user-friendly web applications using Angular and modern web technologies.
</p>

---

## 👩‍💻 About Me

- 💼 Frontend Developer specializing in Angular.
- 🚀 Experienced in building responsive web applications and admin dashboards.
- 🌱 Continuously learning and improving my frontend development skills.
- 💡 Passionate about clean code, reusable components, and modern UI/UX.
- 🤝 Enjoy working in collaborative teams and solving real-world problems.

---

## 🛠️ Tech Stack

<p align="left">
<img src="https://skillicons.dev/icons?i=angular,ts,js,html,css,sass,bootstrap,git,github,vscode,postman,mysql" />
</p>

---

## 📫 Connect With Me

- 📧 **Email:** awadf5584@gmail.com
- 💼 **LinkedIn:** https://www.linkedin.com/in/fatma-khaled-22a420216/
- 🐙 **GitHub:** https://github.com/fatmakhaled21

---

⭐ Feel free to explore my repositories to see my latest projects and contributions.
